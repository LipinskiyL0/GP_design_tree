...
В данном классе реализован стандартный простейший алгоритм генетического программирования
Основные объекты:
    self.n_ind - количество индвидов в популяции
    self.n_iter - количество итераций алгоритма
    self.selection - тип селекции
    self.n_tur - размер турнирной группы
    self.recombination - тип рекомбинации
    self.list_T - список терминальных узлов
    self.list_F - список функциональных узлов
    self.type_ini - метод стартовой инициализации дерева
    self.limit_level - предельный уровень роста дерева
    self.params - список параметров и объектов, которые определяются предметной областью и нужны для рассчета пригодности
    self.transfer_best - флаг переноса лучшего индивида в следующую популяцию
    self.p_mut - вероятность мутации
    
    self.mas_for_mut - массив который используется для совместимости при мутации. Узел можно менять на другой узел с 
                    таким же количеством потомков
    self.cl_tree - класс в котором реализованы механики дерева. Т.е. алгорит оперирует объектами этого класса
    self.parents - популяция родителей
    self.fit_parents - массив пригодностей родителей
    self.childs - популяция потомков
    self.fit_childs - массив пригодностей потомков

    Основные функции и их параметры
    def __init__(self, cl_tree, n_ind=10, n_iter=5, selection="tournament", n_tur=5,
                 recombination="standart", p_mut=0.1, list_T=None, list_F=None, 
                  type_ini='full', limit_level=2, params=None, transfer_best=True ) -> None: конструктор класса
    def opt(self) - основной ход алгоритма
    def fit_function(self, tree, params) - вычисление пригодности
